...
namespace NSCAD
{
    ...
    {
        public static PointF RightPoint(INSCanvas canvas, RectangleF unitrect)
        {
        ...
        public static RectangleF ToScreen(INSCanvas canvas, RectangleF unitrect)
        {
        ...
        public static RectangleF ToUnit(INSCanvas canvas, Rectangle screenrect)
        {
        ...
        public static RectangleF ToScreenNormalized(INSCanvas canvas, RectangleF unitrect)
        {
            RectangleF r = ToScreen(canvas, unitrect);
            r.Y = r.Y - r.Height;
            return r;
        }
        public static RectangleF ToUnitNormalized(INSCanvas canvas, Rectangle screenrect)
        {
            UnitPoint point = canvas.ToUnit(screenrect.Location);
            SizeF size = new SizeF((float)canvas.ToUnit(screenrect.Width), (float)canvas.ToUnit(screenrect.Height));
            RectangleF unitrect = new RectangleF(point.Point, size);
    ...
    public class SelectionRectangle
    {
        ...
        public RectangleF Selection(INSCanvas canvas)
        {
    ...
    {
        List<INSDrawObject> m_originals = new List<INSDrawObject>();
        List<INSDrawObject> m_copies = new List<INSDrawObject>();
        ...
        public IEnumerable<INSDrawObject> Copies
        {
            get { return m_copies; }
        }
        public MoveHelper(Canvas canvas)
        {
            m_canvas = canvas;
        }
        public bool IsEmpty
        ...
        public bool HandleMouseMoveForMove(UnitPoint mouseunitpoint)
        {
            ...
            foreach (INSDrawObject obj in m_copies)
                obj.Move(offset);
        ...
        {
            foreach (INSDrawObject obj in m_originals)
                m_canvas.Model.AddSelectedObject(obj);
            m_originals.Clear();
            m_copies.Clear();
            m_canvas.DoInvalidate(true);
        }
        public void HandleMouseDownForMove(UnitPoint mouseunitpoint, INSSnapPoint snappoint)
        {
            ...
            {
                foreach (INSDrawObject obj in m_canvas.Model.SelectedObjects)
                {
                    m_originals.Add(obj);
    ...
    {
        List<INSNodePoint> _nodes = new List<INSNodePoint>();
        UnitPoint _originPoint = UnitPoint.Empty;
        UnitPoint _lastPoint = UnitPoint.Empty;
        ...
        public RectangleF HandleMouseMoveForNode(UnitPoint mouseunitpoint)
        {
            ...
            r = new RectangleF(_originPoint.Point, new Size(0, 0));
            r = RectangleF.Union(r, new RectangleF(mouseunitpoint.Point, new SizeF(0, 0)));
            if (_lastPoint != UnitPoint.Empty)
                r = RectangleF.Union(r, new RectangleF(_lastPoint.Point, new SizeF(0, 0)));

            _lastPoint = mouseunitpoint;
            foreach (var p in _nodes)
            {
                if (r == RectangleF.Empty)
                    r = p.GetClone().GetBoundingRect(_canvas);
                else
                    r = RectangleF.Union(r, p.GetClone().GetBoundingRect(_canvas));
        ...
        {
            handled = false;
            if (IsEmpty) // no nodes selected yet
            {
                if (_canvas.Model.SelectedCount > 0)
                {
                    foreach (var obj in _canvas.Model.SelectedObjects)
                    {
                        var p = obj.NodePoint(_canvas, mouseunitpoint);
                        if (p == null) continue;
                ...
                handled = _nodes.Count > 0;
                if (handled)
                    _originPoint = mouseunitpoint;
                return handled;
            }

            // update selected nodes
            _canvas.Model.MoveNodes(mouseunitpoint, _nodes);
            _nodes.Clear();
            handled = true;
            _canvas.Canvas.DoInvalidate(true);
            return handled;
        }
        public void HandleCancelMove()
        {
        ...
        public void DrawOriginalObjects(INSCanvas canvas, RectangleF r)
        {
        ...
        public void OnKeyDown(INSCanvas canvas, KeyEventArgs keyevent)
        {
            foreach (var nodepoint in _nodes)
            {
                nodepoint.OnKeyDown(canvas, keyevent);
                if (keyevent.Handled)
                    return;

                var drawobject = nodepoint.GetClone();
                if (drawobject == null)
                    continue;
    ...
    public class Utils
    {
        public static INSDrawObject FindObjectTypeInList(INSDrawObject caller, List<INSDrawObject> list, Type type)
        {
